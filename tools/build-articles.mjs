...
${head}
<meta name="color-scheme" content="light dark"><script>try{const t=localStorage.getItem('reader-theme');if(t==='light'||t==='dark')document.documentElement.dataset.theme=t}catch{}</script>
...
<footer><span>The Comonad.Reader</span><p>${article.rightsNote?esc(article.rightsNote):`Writing and code © ${esc(article.author || 'Edward Kmett')}.<br>Comments attributed to their original authors.`}</p></footer>`;
...
function archivePage(route, period, seriesOnly=false) {
  ...
  return shell({title,base,date:period,route,main:`<header class="archive-header"><p class="article-meta">Edward Kmett &amp; guests</p><h1>${esc(title)}</h1><p>${period?`<a href="${base}reader/">All writing &amp; talks</a>`:"Types, programs, and the structures between them."}</p></header><div class="archive-tools"><label for="archive-search">Explore the archive</label><div class="archive-search-row"><input id="archive-search" type="search" placeholder="Title, topic, speaker, or year…"><select id="archive-kind" aria-label="Content type"><option value="">Everything</option><option>Article</option><option>Talk</option><option>Stream</option><option>Demo</option><option>Paper</option><option${seriesOnly?' selected':''}>Series</option></select></div><p id="archive-count" aria-live="polite">${seriesOnly?countLabel(seriesCatalog.length,'series','series'):countLabel(selected.filter(a=>a.kind==='Article').length,'article')}${seriesOnly?'':' · '+countLabel(selected.filter(a=>a.kind==='Talk').length,'talk')+' · '+countLabel(selected.filter(a=>a.kind==='Paper').length,'paper')+(selected.some(a=>a.kind==='Demo')?' · '+countLabel(selected.filter(a=>a.kind==='Demo').length,'demo'):'')+(selected.some(a=>a.kind==='Stream')?' · '+countLabel(selected.filter(a=>a.kind==='Stream').length,'stream'):'')}</p></div><div class="article-list chronological"${seriesOnly?' hidden':''}>${entries}</div><div class="series-results"${seriesOnly?'':' hidden'}>${seriesCards(base,period)}</div><p id="archive-empty"${selected.length||seriesOnly?' hidden':''}>No matching entries.</p><details class="series-list"><summary>Browse series</summary><p><a href="${base}reader/series/">Explore all series →</a></p><ul><li><a href="${base}reader/series/live-coding/">Live Coding</a></li>${collections.filter(c=>c.links.length).map(c=>`<li><a href="${base+c.path}">${esc(c.title)}</a></li>`).join('')}</ul></details><footer><span>The Comonad.Reader</span><p>Writing and recordings together in date order.<br>Articles and talks credited to their authors and speakers.</p></footer>`,script:`<script type="module" src="${base}archive.js?v=${archiveScriptHash}"></script>`});
}
