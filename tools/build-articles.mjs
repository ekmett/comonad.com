...
<footer><span>The Comonad.Reader</span><p>Writing and code © ${esc(article.author || 'Edward Kmett')}.<br>Comments attributed to their original authors.</p></footer>`;
  ...
  const script = hasPNGDemo ? `<script type="module" src="${root}png-demos.js"></script>` : hasInlineDemo ? `<script type="module" src="${root}article-demos.js"></script>` : article.slug === 'parallel-crc' ? `<script type="module" src="${root}app.js"></script>` : article.slug==='cellular-automata-part-1' ? `<script type="module" src="${root}automaton.js"></script>` : '';
...
const projectSections=Object.entries(packageCatalog.projects||{}).map(([name,project])=>{
 const recordings=[...videos,...publications].filter(v=>(packageCatalog.entryProjects[v.id]||[]).includes(name)).sort(newestFirst);
