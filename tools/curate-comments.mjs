...
for(const post of posts.filter(p=>p.source==='Comonad.Reader')) {
  ...
  for(const node of doc.querySelectorAll('li[id^="comment-"]')) {
    ...
    if(reason)comments.push({id:node.id,author:node.querySelector('cite')?.textContent||'Anonymous',date:node.querySelector('.commentmetadata')?.textContent.trim()||'',markdown:td.turndown(content).replace(/^(\d+)\)/gm,'$1\\)')});
  }
