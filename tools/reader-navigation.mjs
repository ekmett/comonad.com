...
export function readerNavigation(timeline) {
  ...
  function calendar(base, date = years.at(-1)) {
    ...
    return `<aside class="archive-sidebar" aria-label="Archive and blogroll"><details class="archive-disclosure"><summary class="archive-toggle"><span>Archive</span><span class="archive-toggle-icon" aria-hidden="true">‹</span></summary><div class="archive-panel"><nav class="calendar-month calendar-year" aria-label="Archive years">${yearLink(years[yearIndex-1],'Earlier year')}<a href="${base}reader/${year}/">${year}</a>${yearLink(years[yearIndex+1],'Later year')}</nav>${monthGrid}<div class="calendar-posts"><h3>In ${year}</h3>${list}${undated.length?`<details class="archive-month-posts"><summary>Month not recorded <span>${undated.length}</span></summary>${dayList(undated)}</details>`:''}</div><details class="calendar-years"><summary>All years</summary><nav aria-label="All archive years">${[...years].reverse().map(value=>`<a href="${base}reader/${value}/"${year===value?' aria-current="date"':''}>${value}</a>`).join('')}</nav></details><nav class="blogroll" aria-labelledby="blogroll-title"><h3 id="blogroll-title">Blogroll</h3><ul><li><a href="https://bartoszmilewski.com/">Bartosz Milewski<span>Programming Cafe</span></a></li><li><a href="https://blog.sigfpe.com/">Dan Piponi (sigfpe)<span>A Neighborhood of Infinity</span></a></li></ul></nav></div></details></aside>`;
  }
